# Script - Simulador Caixas - versão 1.8 Final - Heurística Otimizada + Comparativo
...
# --- Atualiza sessão ao trocar arquivo ---
...
# --- Função de empacotamento por heurística (FFD ou BFD) ---
def empacotar_produtos(df_base, volume_maximo, peso_maximo, ignorar_braco, converter_pac_para_un, metodo="FFD"):
    ...
    # Define agrupadores conforme flag
    agrupadores = ["ID_Loja"]
    if not ignorar_braco and "Braço" in df_base.columns:
        agrupadores.append("Braço")

    # Agrupa produtos
    grupos = df_base.groupby(agrupadores + ["ID_Produto", "Descrição_produto", "Volume de carga", "Peso de carga", "Unidade med.altern."])[["Qtd.prev.orig.UMA"]].sum().reset_index()

    # Ordena produtos decrescentes (maiores primeiro)
    ...
    for keys, grupo in grupos.groupby(agrupadores):
        ...
        for _, prod in grupo.iterrows():
            ...
            # Alocação nas caixas existentes ou cria nova
